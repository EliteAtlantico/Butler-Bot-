...
def obstacle_footprints(model, data, exclude_body=None, max_bottom=1.6):
    ...
    Per geom, not per body: a body can spread across the whole house. The
    skirting boards in home_search.xml are one `trim` body of 11 strips along
    every wall, and merged into one box they covered every spot in the house.
    Geoms that collide with nothing (that trim, decorative meshes) cannot block
    the robot and are skipped. Things hung higher than `max_bottom` (a lintel)
    are passed under."""
    m, d = model, data
    ...
    for g in range(m.ngeom):
        ...
        if (m.geom_type[g] == mujoco.mjtGeom.mjGEOM_PLANE or not _fixed(m, b)
                or b == exclude_body or (m.geom_contype[g] == 0 and m.geom_conaffinity[g] == 0)):
            continue
