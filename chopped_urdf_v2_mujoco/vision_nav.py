...
"""Object detection + path planning from the simulated RGB-D camera.
    ...
    ./vision_nav.py                      # headless run, writes vision_nav.png
    ./vision_nav.py --viewer             # watch it in the MuJoCo viewer
    ./vision_nav.py --frames             # also dump per-frame perception PNGs

The robot spins once to map the room with its depth camera, finds the red
column with the colour detector, then A*s a path around the barrier it can
see and drives it -- replanning as the map fills in.
"""
...
def parse_args():
    ...
    p.add_argument("--scene", default="scene_nav.xml")
